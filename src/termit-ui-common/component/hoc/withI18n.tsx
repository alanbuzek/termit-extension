...
export default function withI18n<P extends HasI18n = HasI18n>(
  ...
  class Wrapper extends React.Component<
    P & { forwardedRef?: React.Ref<any> } & { intl: IntlShape }
  > {
    protected i18n = (id: string): string =>
      (this.props.intl.messages[id] as string) || `{${id}}`;

    protected formatMessage = (
      msgId: string,
      values: {} | undefined = {}
    ): string => this.props.intl.formatMessage({ id: msgId }, values);

    protected formatDate = (
      value: string | number | Date,
      opts?: FormatDateOptions
    ): string => this.props.intl.formatDate(value, opts);

    protected formatTime = (
      value: string | number | Date,
      opts?: FormatDateOptions
    ): string => this.props.intl.formatTime(value, opts);
  ...
  if (forwardRef) {
    return React.forwardRef<React.ComponentType<P>, P & { intl: IntlShape }>(
      (props: P & { intl: IntlShape }, ref: any) => (
